import { Faculty } from './faculty';

export const facultySeed: Array<Faculty> = [
  {
    id: '20000000-0000-1000-8000-000000000000',
    name: 'Informatics',
    studyPrograms: [
      {
        _id: '20000000-0000-1000-8000-000000000014',
        name: 'Informatics',
      },
      {
        _id: '20000000-0000-1000-8000-000000000015',
        name: 'Informatics: Games Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000016',
        name: 'Information Systems',
      },
      {
        _id: '20000000-0000-1000-8000-000000000017',
        name: 'Bioinformatics',
      },
      {
        _id: '20000000-0000-1000-8000-000000000018',
        name: 'Information Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000019',
        name: 'Data Engineering and Analytics',
      },
      {
        _id: '20000000-0000-1000-8000-000000000020',
        name: 'Robotics, Cognition, Intelligence',
      },
      {
        _id: '20000000-0000-1000-8000-000000000021',
        name: 'Computational Science and Engineering',
      },
    ],
  },
  {
    id: '20000000-0000-1000-8000-000000000001',
    name: 'Mechanical Engineering',
    studyPrograms: [
      {
        _id: '20000000-0000-1000-8000-000000000022',
        name: 'Mechanical Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000023',
        name: 'Engineering Sciences',
      },
      {
        _id: '20000000-0000-1000-8000-000000000024',
        name: 'Automotive Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000025',
        name: 'Energy and Process Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000026',
        name: 'Development, Production and Management in Mechanical Engineering',
      },
      {
        _id: '20000000-0000-1000-8000-000000000027',
        name: 'Mechatronics and Robotics',
      },
      {
        _id: '20000000-0000-1000-8000-000000000028',
        name: 'Medical Engineering and Assistance Systems',
      },
    ],
  },
  {
    id: '20000000-0000-1000-8000-000000000002',
    name: 'TUM School of Management',
    studyPrograms: [
      {
        _id: '20000000-0000-1000-8000-000000000029',
        name: 'Management & Technology',
      },
      {
        _id: '20000000-0000-1000-8000-000000000030',
        name: 'Sustainable Management & Technology',
      },
      {
        _id: '20000000-0000-1000-8000-000000000031',
        name: 'Management',
      },
      {
        _id: '20000000-0000-1000-8000-000000000032',
        name: 'Consumer Science',
      },
      {
        _id: '20000000-0000-1000-8000-000000000033',
        name: 'Finance & Information Management',
      },
    ],
  },
  {
    id: '20000000-0000-1000-8000-000000000003',
    name: 'Mathematics',
    studyPrograms: [
      {
        _id: '20000000-0000-1000-8000-000000000034',
        name: 'Mathematics',
      },
      {
        _id: '20000000-0000-1000-8000-000000000030',
        name: 'Mathematics in Data Science',
      },
      {
        _id: '20000000-0000-1000-8000-000000000031',
        name: 'Mathematical Finance and Actuarial Science',
      },
      {
        _id: '20000000-0000-1000-8000-000000000032',
        name: 'Mathematics in Operations Research',
      },
      {
        _id: '20000000-0000-1000-8000-000000000033',
        name: 'Mathematics in Science and Engineering',
      },
    ],
  },
];
